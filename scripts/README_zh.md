...
> - `inspection-client-windows.zip`（Windows 客户端 exe + `config.json` + `start.bat` / `start_json.bat` / `start_txt.bat` + `README.md` / `README_zh.md`）
>
...
部署前建议在目标服务器运行一次 `_internal/scripts/check_prereqs.ps1`，快速检查补丁状态。
...
`start_json.bat` / `start_txt.bat` 生成的报告文件会输出到根目录。
