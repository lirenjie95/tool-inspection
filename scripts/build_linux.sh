#!/bin/bash
# -*- coding: utf-8 -*-
# Linux 打包脚本 / Linux packaging script
#
# 将 server/agent.py 打包为 ELF 可执行文件，效果类似 Windows 上的 exe，可直接在 Linux 服务器上运行。
# Package server/agent.py as an ELF executable, similar to an exe on Windows, so it can run directly on Linux servers.
#
# 环境要求 / Requirements:
#     - Python 3.7+
...
#
# 输出 / Output: server/dist/inspection-agent/ (文件夹，包含 ELF 可执行文件和依赖 / directory containing the ELF executable and dependencies)

set -e

# 输出语言，通过环境变量 OUTPUT_LANG 设置（zh 或 en，默认 zh）/ Output language, set via OUTPUT_LANG environment variable (zh or en, default zh)
OUTPUT_LANG="${OUTPUT_LANG:-zh}"

# 根据 OUTPUT_LANG 输出中文或英文消息 / Print Chinese or English message based on OUTPUT_LANG
msg() {
...
# 打包 / Package
# --onedir 模式兼容性最好，避免单文件解压问题 / --onedir mode has the best compatibility and avoids single-file extraction issues
...
# 创建 systemd service 文件模板 / Create systemd service file template
# 默认使用 /opt/inspection-agent 作为部署路径，可根据实际情况修改 / Defaults to /opt/inspection-agent as the deployment path; modify as needed
